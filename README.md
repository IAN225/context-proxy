...
一个字段发一次最小请求（十几 token），最后打出「可用 / 被拒」两行，把可用的抄进
`extra_body` 即可。脚本会**先发一个瞎编的字段做对照**：

- 对照被 400 拒掉 → 这家会校验未知字段，那么 OK 的字段就是上游真认识的；
- 对照也返回 200 → 这家对未知字段照单全收，**200 只代表不报错、不代表生效**，
  此时只能看报告里"真的产生了思考内容的组合"这类间接信号来判断。
