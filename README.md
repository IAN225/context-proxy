...
- 主摘要模型与备用摘要模型使用同一组可编辑参数：单次输出上限、token 字段、超时、
  最大尝试次数、最小有效输出长度和 `extra_body` 均可分别配置；最大尝试次数包含首次调用。
- 「检测传参方言」会用不含 token 上限字段的基线请求分别对照 `max_tokens` 与
  `max_completion_tokens`，避免基线与字段探针互相混淆。
- 对于不会拒绝无效字段的宽松网关，只有在不同关闭思考写法产生了明确的“有思考 / 无思考”
  输出差异时，才会把实际生效的写法合并到当前端点的 `extra_body`；摘要端点还会同步
  已验证的 token 上限字段。保存前验证始终使用表单中的最新值。
- 测试明细默认按「发送字段 / 输入 / 输出」展示，并明确标注是否检测到思考；完整请求与
  响应 Payload 放在第二级折叠中。
