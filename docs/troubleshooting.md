...
| `compressing_now` | 只在有请求跑时 >0 | 常年 >0 说明有请求卡住了 |
| `unfinished_compressions` | 会 >0 且能停很久（正常） | 一直不降见 [压缩一直压不完](#压缩一直压不完) |
...
- `/ui` 返回 404 → `server.ui_token` 没配。`./ctl.sh ui-token` 生成一个填进去，`./ctl.sh reload`。
...
`/ui` 的控制台里能看到正在跑的任务（第几批 / 共几批、已压到第几轮、最近一批的输出），
点「中止这个任务」，或者 `./ctl.sh cancel <conv_id 前几位>`。

中止**只在批与批之间生效**：已经落盘的批一条都不撕——钱已经花了，结果留着下次能接着用。
被中止的那次请求多半会返回一条「压缩未完成」的提示（因为没压到阈值以下），
这是预期行为，重发消息就从断点继续。

### 保存模型配置时提示验证失败

控制台保存前会拿这份配置真调一次，调不通就不写文件——写进去下一秒对话就全挂了。
展开「看模型到底吐了什么」，按上游原文改：

- **400/422** → `extra_body` 里有它不认的字段，或者值不合法。删掉那一行，或点「检测传参方言」重新探。
- **401/403** → key 或余额的问题，跟参数无关。
- **429/5xx** → 这次没问上去，跟你的配置不一定有关系。过一会儿再存，或者确信没问题就点「跳过验证强制保存」。
- **HTTP 200 但标了"可疑"** → 中转站把自己的故障当成模型输出发回来了
  （「池子中没有可用账号」这类）。页面不替你下结论，**你自己看那段输出是不是模型说的话**。
...
它一个字段发一次最小请求，最后给出「可用 / 被拒 / 无法判断」清单：

- **「被拒」的字段**（400/422）直接从 `extra_body` 里删掉；
- **「无法判断」的字段**（429 / 5xx / 网络错误，且重试一次仍失败）**别动**——
  那是这次没问出结果，不是上游不认。过一会儿 `--only <字段>` 单独重跑；
- 报告开头会说这家网关到底会不会校验未知字段：
  - **会校验** → 可用项已确认被上游识别，可按需写入 `extra_body`；
  - **不校验** → 可用项仅确认不会报错，是否生效要结合"真的产生了思考内容的组合"、
    `reasoning_tokens` 这些信号判断，别仅凭这一次结果就改 `extra_body`；
  - **无法判断**（对照组自己就撞上了限流或鉴权错误）→ **整份清单都别照着改配置**。

脚本会先发一条不带任何字段的基线请求：它要是就失败了，说明问题在
key / 模型名 / base_url / 余额上，脚本会直接停下并告诉你去查这些，
而不是给你一份"所有字段都不可用"的假清单。
...
把 `observability.capture_timeline` 设为 `true`，`./ctl.sh reload`，让这个会话再发一次消息，
然后在 `/ui` 里点进会话 →「本次请求：进来 → 发出去」→ 加载。会看到：

- 顶部：进来多少条/多少 token → 发出去多少条/多少 token，以及省了百分之几；
- 下面是逐轮时间轴，**灰色 = 已折叠进摘要，蓝色 = 逐字发给上游**，中间一道分隔线。

**只有结构和每轮前 60 字预览，没有原文全文**——原文要看去 chatbox 里看。
几千轮的会话一次只画 60 轮，点「显示更早」翻页，手机上也不会卡。
不看的时候把开关关掉，一分钱开销都没有（实测开着时每请求约 +18ms）。

### 摘要质量不行 / 想手工改

网页版：`http://<服务器IP>:8787/ui`，点进会话直接改。命令行版：
...
想让摘要模型本身压得更好，就调 `summary.prompts.batch`（页面上改会写回 config.yaml 并热重载），
或者换一个更强的 `summary.model`。

### 页面显示「压缩中」但根本没在压 / 摘要改不了

旧版本把 checkpoint 的 `partial` 状态当成"正在压缩"，于是只要上次没压完
（批次上限、摘要模型报错、中转站返回错误页、进程重启……），页面就永久显示压缩中、
摘要永远不让改。现在两件事是分开的：

- `/health` 的 `compressing_now` —— 此刻真有请求在跑。**只有它 >0 才是"压缩中"**，
  页面上摘要也只在这时不让保存。
- `/health` 的 `unfinished_compressions`、会话上的 `partial` —— "上次没压完，
  下次请求接着压"。这是**静止状态**，可以停很久，不影响编辑。

页面上这两者分别显示成「正在压缩」和「没压完」。存在没压完的事件时照样能改摘要，
保存后那些半成品会被标成 `stale` 作废掉——否则下次请求会去续压它，把你手写的内容绕过去。

### 换了摘要之后，模型好像忘了中间一段

大概率是**记忆空洞**：你选的那份摘要只覆盖到第 X 轮，而压缩标记停在第 Y 轮（Y > X），
中间 Y−X 轮既不在摘要里、也不在近期原文窗口里。

这是「设为生效只换内容、不动进度」的必然代价，页面在按钮上和确认框里都会写清楚有多少轮。
两个选择：

- 把那段内容**补进摘要正文**再存（推荐，不花钱）；
- 用 `rewind` 让压缩进度一起退回去，把那段重压一遍：

  ```bash
  curl -X POST -H "Authorization: Bearer $PROXY_UI_TOKEN" -H "Content-Type: application/json" \
       -d '{"rewind": true}' \
       "http://127.0.0.1:8787/admin/session/<id>/checkpoint/<seq>/activate"
  ```

  重压要花钱、也要好几次请求才能压完，别当默认操作用。

### 想固定用某一版摘要

页面的「存档位」里每条都有「设为生效」；接口是
`POST /admin/session/{id}/checkpoint/{seq}/activate`。

指定之后：那份**正文**被复制到链首并固定住（裁剪永远不删），该会话所有没压完的半成品作废，
**重启之后依然是它**，后续压缩也在它之上继续。其它存档还留着，可以再选回去。
直接编辑摘要保存也是同样的效果，不用再点一次。

注意**压缩进度不会跟着回退**（默认 `rewind=false`）：换的是内容，不是进度。
选了一份覆盖范围更早的摘要就会留下记忆空洞，见上一节。
...
`unfinished_compressions` 长期不降，`./ctl.sh sessions` 里某个会话一直 `partial`。
（注意这**不等于**"正在压缩"，也不影响改摘要，见上一节。）
...
| `fallback` | 定位兜底时写下的状态 |
| `manual` | 手工改写摘要写下的（页面或 ./ctl.sh edit） |
